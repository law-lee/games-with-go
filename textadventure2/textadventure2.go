...
func (node *storyNode) render() {
	...
	if node.choices != nil {
		for _, c := range node.choices {
			fmt.Println(c.cmd, ":", c.description)
		}
	}
}
...
func (node *storyNode) executeCmd(cmd string) *storyNode {
	if node.choices != nil {
		for _, c := range node.choices {
			if strings.EqualFold(c.cmd, cmd) {
				return c.nextNode
			}
		}
	}
...
func Run() {
	...
	trap := &storyNode{text: "You head down the well traveled path when suddenly a trap dorr opens and you fall into a pit."}
